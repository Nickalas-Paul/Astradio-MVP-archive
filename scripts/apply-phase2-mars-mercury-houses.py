"""Apply Phase 2 Mars in-sign and Mercury in-house rewrites."""
import json
import re
...
MARS_DATA = DIR / "phase2-mars-signs.json"
MERCURY_HOUSES_DATA = DIR / "phase2-mercury-houses.json"
...
def main() -> None:
    n1 = apply_updates(SIGN_PATH, json.loads(MARS_DATA.read_text(encoding="utf-8")))
    print(f"Updated {n1} entries in {SIGN_PATH.name}")
    n2 = apply_updates(HOUSE_PATH, json.loads(MERCURY_HOUSES_DATA.read_text(encoding="utf-8")))
    print(f"Updated {n2} entries in {HOUSE_PATH.name}")
