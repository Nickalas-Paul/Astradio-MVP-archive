...
export function useCompositionJob(): UseCompositionJobReturn {
  ...
  const pct = 'pct' in currentJob?.status ? currentJob.status.pct : 0;
  const audioUrl = 'url' in currentJob?.status ? currentJob.status.url : undefined;
  const layers = 'layers' in currentJob?.status ? currentJob.status.layers : undefined;
  const error = 'message' in currentJob?.status ? currentJob.status.message : undefined;
