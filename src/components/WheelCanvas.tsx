...
function WheelCanvas({ 
  ...
  const initializeWheel = async () => {
    ...
    const hasValidData = (
      ...
      (chartData.planets && Array.isArray(chartData.planets) && chartData.planets.length > 0)
    ) && (
      (chartData.houses && Array.isArray(chartData.houses) && chartData.houses.length === 12) ||
    ...
    // Create wheel snapshot for the existing wheel.js system
    // Handle both positions object and planets array formats
    let planets;
    if (Array.isArray(chartData.planets)) {
      // Already in planets array format
      planets = chartData.planets;
    } else if (chartData.positions && typeof chartData.positions === 'object') {
      // Convert positions object to planets array
      planets = Object.entries(chartData.positions).map(([id, longitude]) => ({
        id,
        longitude: typeof longitude === 'number' ? longitude : parseFloat(longitude as string) || 0,
      }));
    } else {
      console.warn('WheelCanvas: No valid planets or positions data found');
      return;
    }

    const wheelSnapshot = {
      planets,
      houses: chartData.houses || chartData.cusps || [],
      asc: chartData.asc || chartData.houses?.[0] || chartData.cusps?.[0] || 0,
    };
  ...
  return (
    <div className={`wheel-container ${className}`}>
      ...
      >
        ...
        {/* Empty state */}
        {(!chartData || !chartData.positions || !chartData.houses) && !isLoading && (
