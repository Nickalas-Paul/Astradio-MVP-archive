...
import { useTransportTimeline } from '../core/social/hooks';
...
export function EnhancedTransport({ 
  ...
  useEffect(() => {
    if (!audioRef.current) {
      ...
      audioRef.current.addEventListener('error', (e) => {
        ...
        addToast('Audio playback error. Please try again.', 'error');
        setIsPlaying(false);
      });
  ...
  const play = useCallback(async () => {
    if (audioRef.current) {
      try {
        ...
        addToast('Playing track...', 'info', 2000);
      } catch (error) {
        console.error('Failed to play audio:', error);
        trackError('AUDIO_PLAY_FAILED', 'User interaction required or audio blocked');
        addToast('Failed to play audio. User interaction might be required.', 'error');
      }
    }
  ...
  const pause = useCallback(() => {
    if (audioRef.current) {
      ...
      setIsPlaying(false);
      addToast('Track paused.', 'info', 2000);
    }
  }, [addToast]);
  ...
  const stop = useCallback(async () => {
    if (audioRef.current) {
      ...
      addToast('Track stopped.', 'info', 2000);
    }
  }, [compositionId, addToast]);
