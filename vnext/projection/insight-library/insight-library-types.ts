...
/**
 * Planet-in-sign placement (Phase 5B+). Keys: PLCMT_{PLANET}_{SIGN}.
 */
export interface PlanetInSignInsight {
  readonly key: string;
  readonly planet: string;
  readonly sign: string;
  readonly title: string;
...
export interface AspectInsight {
  ...
  /** Natal self-description (Identity surface): one chart, second person, both planets are the user's. */
  readonly core_self?: string;
  readonly behavioral_self?: string;
