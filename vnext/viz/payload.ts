...
function buildPlan(plan: PlanLike, controls?: ComposeMeta['controls']): VizPayloadPlan {
  ...
  const tempo = bpm / 120; // normalize to ~0.5–1
...
/** Canonical JSON: sort keys recursively for deterministic stringify */
...
/** SHA256 of string (works in browser via crypto.subtle) */
...
/**
 * Build deterministic viz payload from snapshot, plan, compose_meta.
 * Returns { payload, checksum } where checksum = sha256(canonical JSON).
 */
