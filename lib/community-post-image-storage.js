...
async function getCommunityPostImageObject(imageUrl) {
  const key = s3KeyFromImageUrl(imageUrl);
  if (!key || !BUCKET_NAME) {
    const err = new Error('Post image not found');
    err.code = 'POST_IMAGE_NOT_FOUND';
    throw err;
  }
  try {
    ...
    console.error('[community-post-image] S3 get error:', error);
    throw new Error('Failed to fetch post image from storage');
  }
}
