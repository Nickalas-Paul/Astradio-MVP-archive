...
function loadGcpCredentials() {
  ...
  const hasKeys = ['client_email', 'private_key', 'project_id'].every((k) => typeof parsed[k] === 'string');
  if (!hasKeys) {
    console.warn('[GCP] GOOGLE_SERVICE_ACCOUNT_JSON missing required keys (client_email, private_key, project_id); skipping.');
    return;
  }
