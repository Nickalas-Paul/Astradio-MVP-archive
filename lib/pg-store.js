...
async function updateChartBirthFields(chartId, ownerId, input) {
  const row = await getRow('SELECT id, owner_id FROM astradio_charts WHERE id = $1', [chartId]);
...
async function createConnectionIntent(input) {
  ...
  const label = (input.label && String(input.label).trim()) || 'Connection';
  await query(
    `INSERT INTO astradio_connection_intents
      (id, from_user_id, to_user_id, chart_id, from_chart_id, to_chart_id, label, status, created_at)
  ...
  const row = await getRow(
    `SELECT id, from_user_id, to_user_id, chart_id, from_chart_id, to_chart_id, label, status, created_at, accepted_at
...
async function acceptConnectionIntent(intentId, acceptingUserId) {
  const row = await getRow(
    `SELECT id, from_user_id, to_user_id, from_chart_id, to_chart_id, label, status
     FROM astradio_connection_intents WHERE id = $1`,
    [intentId]
  );
  ...
  const label = (row.label && String(row.label).trim()) || 'Connection';
  const t = now();
