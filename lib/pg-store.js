...
// ---- Exports ----
/**
 * Daily composition limit: find most recent export for user+source created today (UTC calendar date).
 * Uses export_meta->>'source' on astradio_export_jobs (no dedicated source column).
 * @param {string} userId
 * @param {string} source e.g. 'sky' | 'profile_active'
 * @returns {Promise<{ export_id: string } | null>}
 */
async function findTodayExportForUser(userId, source) {
  const uid = String(userId || '').trim();
  const src = String(source || '').trim();
  if (!uid || !src) return null;
  try {
    const row = await getRow(
      `SELECT id FROM astradio_export_jobs
       WHERE user_id = $1
         AND export_meta->>'source' = $2
         AND created_at >= date_trunc('day', NOW() AT TIME ZONE 'UTC')
       ORDER BY created_at DESC
       LIMIT 1`,
      [uid, src]
    ...
    return { export_id: String(row.id) };
  } catch (e) {
...
/**
 * Record a daily export pointer after successful Lyria compose (idempotent on export id).
 * @param {string} userId
 * @param {string} source
 * @param {string} exportId
 * @param {object} [opts]
 */
async function recordTodayExportForUser(userId, source, exportId, opts = {}) {
  const uid = String(userId || '').trim();
  const src = String(source || '').trim();
  const eid = String(exportId || '').trim();
  if (!uid || !src || !eid) return null;
  const exportMeta = {
    ...(opts.exportMeta && typeof opts.exportMeta === 'object' ? opts.exportMeta : {}),
    source: src,
    daily: true,
  };
  try {
    return await createExportJob({
      ...
      chartHash: opts.chartHash || null,
      filePath: opts.filePath || `${eid}.wav`,
    ...
    if (e.code === '23505') return getExportJob(eid);
    throw e;
  }
}
...
/**
 * @param {object} input
 * @param {string} [input.contentType='audio/wav'] MIME type stored for format negotiation on GET /api/exports/:id
 */
