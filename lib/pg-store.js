...
async function createCommunityPost(input) {
  ...
  await query(
    `INSERT INTO community_posts (id, user_id, title, body, moderation_status, created_at, updated_at)
     VALUES ($1, $2, $3, $4, $5, $6, $7)`,
    [id, input.userId, input.title || '', input.body || '', moderationStatus, t, t]
  );
...
async function getCommunityPost(id) {
  const row = await getRow(
    'SELECT id, user_id, title, body, moderation_status, created_at, updated_at FROM community_posts WHERE id = $1',
    [id]
  );
...
async function listCommunityPostsFeed(opts = {}) {
  ...
  const rows = await getRows(
    `SELECT p.id, p.user_id, p.title, p.body, p.moderation_status, p.created_at, p.updated_at
     FROM community_posts p
     LEFT JOIN community_user_settings s ON s.user_id = p.user_id
     WHERE COALESCE(s.public_visibility, true) = true
       AND p.moderation_status IN ('passed', 'skipped', 'error')
...
async function countCommunityPostsFeed() {
  const row = await getRow(
...
async function listCommunityPostsByUser(userId, opts = {}) {
  const limit = Math.min(50, Math.max(1, opts.limit || 20));
  const rows = await getRows(
    `SELECT id, user_id, title, body, moderation_status, created_at, updated_at
     FROM community_posts
     WHERE user_id = $1 AND moderation_status IN ('passed', 'skipped', 'error')
...
async function getCommunityComment(id) {
  const row = await getRow(
    'SELECT id, post_id, user_id, body, created_at, updated_at FROM community_comments WHERE id = $1',
    [id]
  );
...
async function upsertCommunityUserSettings(userId, input) {
  const t = now();
  const bio = input.bio != null ? String(input.bio) : '';
  ...
  await query(
    ...
    [userId, bio, publicVisibility, JSON.stringify(keywords), t]
  );
  return getCommunityUserSettings(userId);
}

async function getCommunityPublicProfile(userId) {
  const user = await getUser(userId);
  if (!user) return null;
  const settings = await getCommunityUserSettings(userId);
  if (!settings.publicVisibility) return { userId, publicVisibility: false };
...
async function listSignalsForSender(userId) {
  try {
    ...
    return rows.map((r) => ({
      ...
      recipientDisplayName: r.recipient_display_name || r.recipient_user_id,
      anchorType: r.anchor_type,
      anchorId: r.anchor_id,
      templateId: r.template_id,
      body: r.body_json,
