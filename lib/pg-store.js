 ...
 *  - Primary: real users with primary chart and discoverable=true.
 *  - Fallback (pre-migration / dev without visibility columns): match candidates only.
 *  Search UI should not show demo placeholders when real users exist.
 */
async function listDirectoryEligibleUsers() {
  // First, try to use real users from Postgres (preferred path in production/preview).
  try {
    const rows = await getRows(
    ...
    // If there are zero real users yet (fresh DB), return empty list so search shows "no users" rather than demo placeholders.
    return [];
  } catch (e) {
    if (!isMissingVisibilityColumnError(e)) throw e;
    // Migration 011 not applied: fall through to demo-only directory.
  }

  // Fallback path: match candidates only (older environments without discoverability columns).
