...
async function createUser(input) {
  ...
  try {
    ...
    if (e?.message && /column "discoverable" does not exist/i.test(e.message)) {
      await query(
...
/** Phase 8G: Users eligible for directory search (match candidates + real users with primary chart and discoverable=true). */
async function listDirectoryEligibleUsers() {
  ...
  try {
    ...
    if (e?.message && /column "discoverable" does not exist/i.test(e.message)) {
      // Migration 011 not applied; only match candidates are eligible
    } else {
      throw e;
    }
  }
...
async function listRecentDiscoverableUsers(limit = 20) {
  try {
    ...
    if (e?.message && /column "show_in_feed" does not exist/i.test(e.message)) return [];
    throw e;
  }
}
...
async function updateUserDiscoverability(userId, { discoverable, show_in_feed }) {
  ...
  try {
    ...
    if (e?.message && /column "discoverable" does not exist/i.test(e.message)) return;
    throw e;
  }
}
