 ...
 * ONLY rows with source = community_relational_weather; does not insert; does not touch other sources.
 */
async function repairCommunityRelationalWeatherLibraryComposition(input) {
  ...
  if (!ownerUserId || !objectIdentityHash) return { updated: false };
  ...
  const newExport =
    input.exportJobId != null && String(input.exportJobId).trim() ? String(input.exportJobId).trim() : null;

  const updateText = text != null || (weather != null && typeof weather === 'object');
  const updateExport = !!(newExport && newExport.length === 64 && /^[a-f0-9]{64}$/.test(newExport));

  if (!updateText && !updateExport) return { updated: false };
  ...
  if (updateText && updateExport) {
    const res = await query(
      `UPDATE astradio_sandbox_compositions
       SET report = $1::jsonb,
           export_id = COALESCE(NULLIF($2::text, ''), export_id),
           updated_at = NOW()
       WHERE id = $3 AND owner_user_id = $4 AND source = $5`,
      [JSON.stringify(reportObj), newExport, row.id, ownerUserId, 'community_relational_weather']
    );
    return { updated: (res.rowCount || 0) > 0 };
  }
  if (updateText) {
  ...
  const res = await query(
    `UPDATE astradio_sandbox_compositions
     SET export_id = COALESCE(NULLIF($1::text, ''), export_id), updated_at = NOW()
     WHERE id = $2 AND owner_user_id = $3 AND source = $4`,
    [newExport, row.id, ownerUserId, 'community_relational_weather']
  );
  return { updated: (res.rowCount || 0) > 0 };
}
