...
async function createCommunityPost(input) {
  ...
  const t = now();
  await query(
    `INSERT INTO community_posts (id, user_id, title, body, created_at, updated_at)
     VALUES ($1, $2, $3, $4, $5, $6)`,
    [id, input.userId, input.title || '', input.body || '', t, t]
  );
...
async function getCommunityPost(id) {
  const row = await getRow(
    'SELECT id, user_id, title, body, created_at, updated_at FROM community_posts WHERE id = $1',
    [id]
  );
...
async function listCommunityPostsByUser(userId, opts = {}) {
  ...
  const rows = await getRows(
    `SELECT id, user_id, title, body, created_at, updated_at
     FROM community_posts WHERE user_id = $1 ORDER BY created_at DESC LIMIT $2`,
    [userId, limit]
  );
  return rows.map(rowToCommunityPost);
}
