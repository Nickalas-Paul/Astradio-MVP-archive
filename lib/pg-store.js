...
async function getUser(id) {
  try {
    const row = await getRow(
      'SELECT id, handle, display_name, email, discoverable, show_in_feed, bio, avatar_url, discoverable_as, looking_for, created_at, updated_at FROM astradio_users WHERE id = $1',
      [id]
    );
...
async function getUserByHandle(handle) {
  try {
    const row = await getRow(
      'SELECT id, handle, display_name, email, discoverable, show_in_feed, bio, avatar_url, discoverable_as, looking_for, created_at, updated_at FROM astradio_users WHERE handle = $1',
      [handle]
    );
    if (!row) return undefined;
    return rowToUser(row);
...
async function listUsers() {
  try {
    const rows = await getRows(
      'SELECT id, handle, display_name, email, discoverable, show_in_feed, bio, avatar_url, discoverable_as, looking_for, created_at, updated_at FROM astradio_users ORDER BY created_at'
    );
...
async function listDirectoryEligibleUsers() {
  ...
  try {
    const rows = await getRows(
      `SELECT u.id, u.handle, u.display_name, u.bio, u.avatar_url, u.discoverable_as, u.looking_for, pc.chart_id
