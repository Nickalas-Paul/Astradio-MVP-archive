...
function isProxySecretExempt(req) {
  ...
  // TikTok OAuth: browser redirects cannot send x-proxy-secret
  const reqPath = original;
  if (reqPath.startsWith('/api/social/tiktok/authorize')) return true;
  if (reqPath.startsWith('/api/social/tiktok/callback')) return true;
