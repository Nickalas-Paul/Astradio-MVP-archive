...
function createChartMem({ ownerId, label, date, time, lat, lon, tz }) {
...
async function createChart(input) {
  if (pgStore) return pgStore.createChart({ ...input, timezone: input.tz });
  return Promise.resolve(createChartMem(input));
}
