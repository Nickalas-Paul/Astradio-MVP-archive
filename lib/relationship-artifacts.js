...
async function resolvePairParticipantUserIds(chartIdLow, chartIdHigh) {
  const lowChart = await pgStore.getChart(chartIdLow);
  const highChart = await pgStore.getChart(chartIdHigh);
  return Array.from(
    new Set(
...
async function materializeRelationshipArtifact(relationshipId, ownerUserId) {
  ...
  let newExportIdFromCreate = null;
  if (!comparisonId) {
    ...
    if (!cmpResult.comparison || !cmpResult.comparison.id) {
      throw new Error('materializeRelationshipArtifact: createComparison did not persist comparison');
    }
    comparisonId = cmpResult.comparison.id;
    newExportIdFromCreate = cmpResult.exportId ? String(cmpResult.exportId) : null;
    ...
    if (newExportIdFromCreate) {
      await pgStore.updateComparisonExportJobIfEmpty(comparisonId, newExportIdFromCreate);
    }
  }
  ...
  if (comparisonId) {
    if (newExportIdFromCreate) {
      exportJobId = newExportIdFromCreate;
      comparisonRow = await pgStore.getComparison(comparisonId);
    } else {
      comparisonRow = await pgStore.getComparison(comparisonId);
      exportJobId = comparisonRow && comparisonRow.exportJobId ? String(comparisonRow.exportJobId) : null;
    }
  }

  if (comparisonId) {
    const participantUserIds = await resolvePairParticipantUserIds(
      relationship.chartIdLow,
      relationship.chartIdHigh
    );
    await pgStore.ensureCommunityRelationshipLibraryEntriesForParticipants({
      participantUserIds,
      chartIdLow: relationship.chartIdLow,
      chartIdHigh: relationship.chartIdHigh,
      label: relationship.label,
      comparisonId,
      exportJobId,
      compositionId: comparisonRow?.compositionId || null,
      planHash: comparisonRow?.planHash || null,
    });
  }
