...
const { getRow, getRows, insert, remove } = require('../lib/database');
...
// Follow user
router.post('/follows/:username', authenticateToken, async (req, res) => {
  try {
...
// Unfollow user
router.delete('/follows/:username', authenticateToken, async (req, res) => {
  try {
    const { username } = req.params;
...
router.get('/follows/:username/followers', optionalAuth, async (req, res) => {
  try {
    const { username } = req.params;
...
router.get('/follows/:username/following', optionalAuth, async (req, res) => {
  try {
    const { username } = req.params;
    const { page = 1, limit = 20 } = req.query;
    const offset = (page - 1) * limit;
