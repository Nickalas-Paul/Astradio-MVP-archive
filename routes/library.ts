// routes/library.ts
// TypeScript migration of library module (H3.2)
// Strangler pattern: maintains same API surface as library.js
...
const { authenticateToken } = authLib;
...
router.get('/saved', authenticateToken, async (req: any, res: Response, next: NextFunction) => {
  try {
    ...
    res.json({
      ...
      pagination: {
        ...
        pages: Math.ceil(parseInt(countResult.count) / parseInt(limit as string)),
      },
    });
  } catch (error) {
...
router.get('/liked', authenticateToken, async (req: any, res: Response, next: NextFunction) => {
  try {
    const { page = 1, limit = 20 } = req.query;
    const offset = (parseInt(page as string) - 1) * parseInt(limit as string);
...
// Create playlist
router.post('/playlists', validatePlaylist, authenticateToken, async (req: any, res: Response, next: NextFunction) => {
  try {
...
router.get('/playlists', authenticateToken, async (req: any, res: Response, next: NextFunction) => {
  try {
    const { page = 1, limit = 20 } = req.query;
    const offset = (parseInt(page as string) - 1) * parseInt(limit as string);
...
// Get playlist by ID
router.get('/playlists/:id', authenticateToken, async (req: any, res: Response, next: NextFunction) => {
  try {
    const { id } = req.params;
...
// Update playlist
router.patch('/playlists/:id', validatePlaylist, authenticateToken, async (req: any, res: Response, next: NextFunction) => {
  try {
    const errors = validationResult(req);
    if (!errors.isEmpty()) {
      return res.status(400).json({ errors: errors.array() });
    ...
    const updatedPlaylist = await update(query, values) as Playlist;
...
// Delete playlist
router.delete('/playlists/:id', authenticateToken, async (req: any, res: Response, next: NextFunction) => {
  try {
    const { id } = req.params;
...
router.post('/playlists/:id/items', [
  ...
  try {
    ...
    const track = await getRow('SELECT id FROM tracks WHERE id = $1', [track_id]) as { id: string };
    if (!track) {
...
router.delete('/playlists/:id/items/:trackId', authenticateToken, async (req: any, res: Response, next: NextFunction) => {
  try {
...
router.patch('/playlists/:id/reorder', [
  ...
  try {
    ...
    const { id } = req.params;
    const { track_positions } = req.body as { track_positions: TrackPosition[] };
